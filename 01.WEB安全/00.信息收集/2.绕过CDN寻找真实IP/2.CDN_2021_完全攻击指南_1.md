...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609859142310.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/cdn.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609835365555.png)

IP历史：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609837315105.png)

子域名：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609837340076.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609835942920.png)

3.WhoISrequest （https://whoisrequest.com/）

WhoISrequest 这个网站自2002年以来一直在跟踪和记录DNS历史变更，数据底蕴很足。

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609836305819.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609842380777.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609842464853.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609842645459.png)

觉得难？不想学？那也没关系，站长已经考虑到你这种懒人情况了，首页就有给懒人准备的搜索构造器，十分友好：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609842713752.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609842762873.png)

10.SubDomainTools（https://ruo.me/sub）

在线子域名查询，支持实时模式和后台模式，不阻塞前端线程，不占CPU，小测试的时候非常方便。

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609843721712.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609844179805.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609843385119.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609844408404.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609844734168.png)

4.Zoomeye

Zoomeye，钟馗之眼，知道创宇打造的宇宙级网络空间搜索引擎，Shodan 侧重于主机设备，Zoomeye 则偏向于 Web 发现。

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609845029003.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609847222894.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609851611789.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609852147581.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609854276935.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609892872918.png)

subdomain 功能支持暴力破解模式和非暴力模式，可以只探测 web 服务或者只探测 ip 可解析的子域，另外支持 webhook 数据传递，作为插件端或者分发消息的接收端来使用都非常方便快捷。

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609896788865.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609918686608.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609867423224.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609898369697.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609899654154.png)

同理，我们根据域名指纹搜索 SSL 证书指纹，然后再反查 IP 就可以了。

首先针对目标域名，选择 `Certificates` 证书搜索，结果出来一堆 `有效` 的证书和 `无效 ` 的证书：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609900222284.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609900562834.png)

瞬间减少到了 2 个结果，我们逐个点开，然后 sha1 签名反查 IPV4 主机：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609900755052.png)

不过很遗憾，两条记录全都没查到：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609900799143.png)

这也从侧面说明了，不是所有的网站只有其 `有效` 证书会匹配其 IP 服务器。

于是只能回到最开始，从所有的结果里面依次打开往下反查指纹，最终定位到了真实 IP ：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609901014696.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609901819446.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609902562508.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609912655696.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609921963504.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609921974193.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609922128990.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609922290200.png)

也可以使用海外的超级ping类的平台在线多国家多地区测试，推荐 https://www.host-tracker.com/v3/check/ ，支持 140 多个地区的 ping 测试，另外支持针对某子域超长期的监控，通过邮件通知，简直太强大了。

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609940418986.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609940432699.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609924326577.png)
...
```
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609924510054.png)

针对整个互联网的搜索 IP 结果进一步匹配，比如通过 端口、地理位置、banner 特征、反查邮箱、联系方式等等信息，继续精确结果：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609924718669.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609924903773.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609936601531.png)

刷新一下 dnslog ，获得了真实服务器 IP：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609937441252.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609947516374.png)

IP 正确，子域正确：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609947462269.png)


IP 错误，子域正确：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609947608941.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609949974253.png)
...
```
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609979138336.png)

另外值得一提的是，渗透过程中同样可以根据 favicon 哈希、子域、IP 的命中，做好指纹排序并整理，另外做一张常见的中间件或者组件指纹哈希表，分类进行资产快速命中：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609981902217.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1609981925486.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609987888510.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609988013424.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1609989480487.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1610008247459.png)

喜欢偷懒的同学可以使用在线查询：`https://suip.biz/?act=cloudfail` 已经将此工具集成到了后台，并做了前端 UI 页面供安全信息从业人员在线查询：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1610008613919.png)

![Alt text](CDN 2021 完全攻击指南（一）.assets/1610008639802.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1610008845069.png)

稍等一段时间就可以了，这里也给大家截个图，防止有同学误会工具不可以使用：

![Alt text](CDN 2021 完全攻击指南（一）.assets/1610010599215.png)
...
![Alt text](CDN 2021 完全攻击指南（一）.assets/1610010937798.png)
